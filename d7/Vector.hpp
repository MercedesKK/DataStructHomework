...
namespace MercedesKK
{
	...
	*/
	...
	template <typename value_type>
	Vector<value_type>::Vector(const Vector& vec)
	{
		_size = vec._size;
	...
	template <typename value_type>
	Vector<value_type>& Vector<value_type>::operator=(const Vector& vec)
	{
		if (this == &vec)
			return *this;
	...
	template <typename value_type>
	void Vector<value_type>::insert(iterator it, value_type val)
	{
		int index = it - _data;
		if (0 == _capacity) {
			_capacity = 1;
	...
	template <typename value_type>
	bool Vector<value_type>::operator==(const Vector& vec) const
	{
		if (_size != vec._size)
			return false;
		for (int i = 0; i < _size; i++)
